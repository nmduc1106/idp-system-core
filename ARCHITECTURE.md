...
- **Health Check:** `pg_isready` with 10 s interval, 5 retries.
...
1. Client sends a `POST /api/v1/upload` request with a document file.
2. JWT Auth Middleware validates the request token (supports both `Authorization` header and HttpOnly cookies).
3. The API Gateway uploads the file to **MinIO** object storage.
4. Document metadata (filename, upload timestamp, status) is persisted to **PostgreSQL**.
5. A processing job message is published to the **RabbitMQ** queue.
6. The client receives an immediate acknowledgement with a document tracking ID.
...
docker-compose up --scale ocr-worker=3 -d   # Scale OCR Workers
```
...
- **Build:** Multi-stage Dockerfiles for minimal production images.
